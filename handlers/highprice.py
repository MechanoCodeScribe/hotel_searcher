...
from states.states_classes import HighState
...
def highprice_selected(message: types.Message) -> None:
    """
        Initiates the HighState.location state when the /highprice command is selected.

        :param message: The message object representing the user's command.
        :type message: telebot.types.Message
        :return: None
    """
    bot.set_state(user_id=message.from_user.id, state=HighState.location, chat_id=message.chat.id)
    logger.info('Command /lowprice. Starting state.')
    API.payload_list['filters']['price']['min'] = 10
    API.payload_list['filters']['price']['max'] = 30000
    API.payload_list['sort'] = "PRICE_HIGH_TO_LOW"
    API.payload_list['resultsSize'] = 100
...
@logger.catch()
@bot.message_handler(state=HighState.location)
def highprice_location(message: types.Message) -> None:
    """
...
def highprice_request_hotels(call):
    """
    ...
    bot.set_state(user_id=call.from_user.id, state=HighState.offers_count, chat_id=call.message.chat.id)
...
def highprice_return(call):
    ...
    highprice_selected(call.message)


@logger.catch()
@bot.message_handler(state=HighState.offers_count)
def highprice_check_hotels(message: types.Message) -> None:
    """
        ...
        :return: None
    """
    check_hotels(message=message, state=HighState.clients_count)


@logger.catch()
@bot.message_handler(state=HighState.clients_count)
def highprice_start_calendar(message: types.Message) -> None:
    """
...
def highprice_handle_calendar(call):
    """
    ...
    if res:
        ...
        calendar_part1(message=call.message, cal_id=4, min_date=API.min_date + timedelta(days=1), max_date=API.min_date + timedelta(days=180))


@logger.catch()
@bot.callback_query_handler(func=DetailedTelegramCalendar.func(calendar_id=4))
def highprice_end_calendar(call):
    """
        ...
        :return: None
    """
    res = calendar_part2(call, 4, min_date=API.min_date + timedelta(days=1), max_date=API.min_date + timedelta(days=180))
    if res:
        date_list = str(res).split('-')
        with bot.retrieve_data(user_id=call.from_user.id, chat_id=call.message.chat.id) as data:
        ...
        pic_markup = pictures_question('get2')
        bot.send_message(chat_id=call.from_user.id, text='Would you like to see pictures of the hotels?', reply_markup=pic_markup)


@logger.catch()
@bot.callback_query_handler(func=lambda call: call.data.startswith('get2'))
def highprice_photo_handler(call: types.CallbackQuery):
    """
        Handles the user's choice (whether photos are needed or not).
        Proceeds to the highprice_photos_question function for API request.

        :param call: The callback object representing the user's interaction.
        :type call: telebot.types.CallbackQuery
        :return: None
    """
    photos_needed = call.data == 'get2_yes'
    highprice_photos_question(call, photos_needed)


@logger.catch()
def highprice_photos_question(call, photos_needed):
    """
        ...
        If photos are needed, changes the state and proceeds to the highprice_pictures_confirmed function.
    ...
    if photos_needed:
        ...
        bot.set_state(user_id=call.from_user.id, state=HighState.pictures_count, chat_id=call.message.chat.id)
        ...
        if not get_data:
            ...
            make_api_request1(message=call.message, id_price=id_price, distance=distance, data=data, user_id=call.from_user.id)
        bot.delete_state(call.from_user.id, call.message.chat.id)


@logger.catch()
@bot.message_handler(state=HighState.pictures_count)
def highprice_pictures_confirmed(message: types.Message) -> None:
    """
